...
public class PrivateKey {
    ...
    private static final String              MIXING_CONSTANT             = "mixing-constant";

    private static final int                 DEFAULT_CHAIN_LENGTH        = 2;                                          // The
                                                                                                                        // value
                                                                                                                        // of
                                                                                                                        // this
                                                                                                                        // constant
    ...
    private final BitVector                  mixingConstant;
    ...
    @JsonCreator
    public PrivateKey(
            @JsonProperty( D_PROPERTY ) EnhancedBitMatrix D,
            @JsonProperty( L_PROPERTY ) EnhancedBitMatrix L,
            @JsonProperty( E1_PROPERTY ) EnhancedBitMatrix E1,
            @JsonProperty( E2_PROPERTY ) EnhancedBitMatrix E2,
            @JsonProperty( A_PROPERTY ) EnhancedBitMatrix A,
            @JsonProperty( B_PROPERTY ) EnhancedBitMatrix B,
            @JsonProperty( F_PROPERTY ) SimplePolynomialFunction F,
            @JsonProperty( G_PROPERTY ) SimplePolynomialFunction G,
            ...
            @JsonProperty( MIXING_CONSTANT ) BitVector mixingConstant,
        ...
        this.mixingConstant = mixingConstant;
    ...
    public PrivateKey( int cipherTextBlockLength, int plainTextBlockLength, int complexityChainLength ) {
        ...
        while ( !initialized && ( ( --rounds ) != 0 ) ) {
            ...
            try {
                ...
                Preconditions.checkState(
                        dgen.multiply( e2gen ).isIdentity(),
        ...
        A = Agen;
        B = Bgen;
        complexityChain = SimplePolynomialFunctions.arrayOfRandomMultivariateQuadratics(
                plainTextBlockLength,
                plainTextBlockLength,
        ...
        mixingConstant = BitVectors.randomVector( plainTextBlockLength );
        try {
    ...
    public SimplePolynomialFunction encryptBinary( SimplePolynomialFunction plaintextFunction ) {
        ...
        SimplePolynomialFunction R = SimplePolynomialFunctions.randomFunction(
                plaintextFunction.getInputLength(),
                plaintextLen );
    ...
    public SimplePolynomialFunction encrypt( SimplePolynomialFunction input, SimplePolynomialFunction g ) {
        Pair<SimplePolynomialFunction, SimplePolynomialFunction[]> pipeline = SimplePolynomialFunctions
                .buildNonlinearPipeline(
                        g.xor( SimplePolynomialFunctions.constant( E1.rows(), mixingConstant ) ),
                        complexityChain );

        SimplePolynomialFunction E = E1.multiply( input.xor( A.multiply( g ) ) ).xor(
                E2.multiply( input.xor( B.multiply( g ) ) ) );
        return E.xor( ParameterizedPolynomialFunctions.fromUnshiftedVariables(
                g.getInputLength(),
                E1.multiply( pipeline.getLeft() ).xor( E2.multiply( pipeline.getLeft() ) ),
                pipeline.getRight() ) );
    }
    ...
    @JsonProperty( MIXING_CONSTANT )
    public BitVector getMixingConstant() {
        return mixingConstant;
    }
    ...
    public EnhancedBitMatrix randomizedL() throws SingularMatrixException {
        ...
        return randomL.multiply( Preconditions.checkNotNull( E1, "E1 must not be null." ) ).inverse()
                .multiply( randomL ); // Normalize
    }
    ...
    public SimplePolynomialFunction buildDecryptor() throws SingularMatrixException {
        /*
         * G( x ) = Inv( A + B ) (L + D) x \mathcal D( x ) = L x + A G( x ) + c'_1 h'_1 + c'_2 h'_2
         */
        ...
        SimplePolynomialFunction DofX = L
                ...
                .xor( A.multiply( GofX ) )
                .xor(
                        ParameterizedPolynomialFunctions.fromUnshiftedVariables(
                                GofX.getInputLength(),
                                pipeline.getLeft(),
                                pipeline.getRight() ) );
        return DofX;
    ...
    public SimplePolynomialFunction buildMirroredDecryptor() throws SingularMatrixException {
        ...
        SimplePolynomialFunction DofX = mirror( L.multiply( X ).xor( A.multiply( GofX ) ) ).xor(
                ParameterizedPolynomialFunctions.fromUnshiftedVariables(
                        GofX.getInputLength() << 1,
                        mirror( pipeline.getLeft() ),
                        mirroredPipelines ) );
    ...
    public boolean equals( Object obj ) {
        ...
        if ( !Arrays.equals( complexityChain, other.complexityChain ) ) return false;
        if ( decryptor == null ) {
            if ( other.decryptor != null ) return false;
        } else if ( !decryptor.equals( other.decryptor ) ) return false;
        if ( longsPerBlock != other.longsPerBlock ) return false;
        if ( mirroredDecryptor == null ) {
            if ( other.mirroredDecryptor != null ) return false;
        } else if ( !mirroredDecryptor.equals( other.mirroredDecryptor ) ) return false;
        return true;
    }
}
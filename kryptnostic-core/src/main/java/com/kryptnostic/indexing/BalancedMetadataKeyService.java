 ...
 * This class is used to compute the appropriate location to store a token's metadata.
 * 
 */
public class BalancedMetadataKeyService implements MetadataKeyService {
	...
	public String getKey(String token, BitVector nonce) {
		...
		for( int i = 0 ; i < rawResult.length ; ++i ) {
			buf.putLong( rawResult[ i ] );
		}
		
		return new String( codec.encode( buf.array() ) );
	}
	...
	public Metadata mapTokensToKeys(Set<Metadatum> metadata) {
		 ...
		 * 
		 *  Generally, the list of metadatum should be of length one, but in rare cases collisions may occur.
		 *  In the case of a collision we'll just store both at the same location. In the future, we may want
		 *  to have a specific number of retries before giving up and allowing a collision. 
		 *  
		 *  In theory this shouldn't be a security risk, because its hard for an attacker to force stuff into 

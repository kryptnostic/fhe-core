...
public class OptimizedPolynomialFunctionGF2 extends BasePolynomialFunction {
	...
	protected static final int CONCURRENCY_LEVEL = Runtime.getRuntime()
			.availableProcessors() - 1;
	protected static final ListeningExecutorService executor = MoreExecutors
			.listeningDecorator(Executors.newFixedThreadPool(CONCURRENCY_LEVEL));

	public OptimizedPolynomialFunctionGF2(int inputLength, int outputLength,
	...
	public SimplePolynomialFunction compose(SimplePolynomialFunction inner) {
		...
		BitVector[] results = expandOuterMonomials(prereqs.monomialsList,
				prereqs.innerRows, prereqs.indices);

		return postProcessCompose(prereqs.monomialsList, prereqs.indices,
				results, inner);
	}

	@Override
	protected BitVector[] expandOuterMonomials(final List<Monomial> mList,
				...
				public void run() {
					for (int j = fromIndex; j < toIndex; j++) {
						...
						if (outerMonomial.isZero()) {
							...
							for (int i = Long
									.numberOfTrailingZeros(outerMonomial
											.elements()[0]); i < outerMonomial
									.size(); ++i) {
								if (outerMonomial.get(i)) {
									if (newContributions == null) {
										newContributions = innerRows[i];
									} else {
										newContributions = product(
												newContributions, innerRows[i],
												mList, indices);
									}
								}

...
public class PolynomialFunctionTests {
	private static final Logger logger = LoggerFactory
			.getLogger(PolynomialFunctionTests.class);
	...
	public void builderTest() {
		OptimizedPolynomialFunctionGF2.Builder builder = OptimizedPolynomialFunctionGF2
				.builder(256, 256);
		for (int i = 0; i < 1024; ++i) {
			BitVector contribution = BitUtils.randomVector(256);
			builder.setMonomialContribution(Monomial.randomMonomial(256, 4),
					contribution);
		}
	...
	public void denseRandomMVQTest() {
		...
		Assert.assertEquals(
				1
						+ f.getInputLength()
						+ ((f.getInputLength() * (f.getInputLength() - 1)) >>> 1),
	...
	public void monomialSetProductTest() {
		...
		Set<Monomial> actual = OptimizedPolynomialFunctionGF2.product(rowA,
				rowB);
	...
	public void generalComposeTest() {
		SimplePolynomialFunction outer = PolynomialFunctions.randomFunction(
				INPUT_LENGTH, OUTPUT_LENGTH, 10, 3);
		SimplePolynomialFunction inner = PolynomialFunctions.randomFunction(
				INPUT_LENGTH, INPUT_LENGTH, 10, 2);
	...
	public void testConcatenateOutputs() {
		...
		SimplePolynomialFunction lhs = PolynomialFunctions
				.denseRandomMultivariateQuadratic(inputLength, outputLength);
		SimplePolynomialFunction rhs = PolynomialFunctions
				.denseRandomMultivariateQuadratic(inputLength, outputLength);

		SimplePolynomialFunction concatenated = PolynomialFunctions
				.concatenate(lhs, rhs);
	...
	public void testToFromString() {
		...
		SimplePolynomialFunction fPrime = FunctionUtils.fromString(
				f.getInputLength(), fString);
	...
	public void testRandomlyPartitionMVQ() {
		...
		Pair<SimplePolynomialFunction, SimplePolynomialFunction> gh = PolynomialFunctions
	...
	public void testPipelineStage() {
		...
		SimplePolynomialFunction f = PolynomialFunctions
				.denseRandomMultivariateQuadratic(inputLength, outputLength);
		SimplePolynomialFunction inner = PolynomialFunctions
				.randomManyToOneLinearCombination(inputLength);
		PolynomialFunctionPipelineStage stage = PolynomialFunctionPipelineStage
				.build(f, inner);
		...
		BitVector concatenatedInput = FunctionUtils.concatenate(inputLower,
				inputUpper);

		Assert.assertEquals(
				inputLower,
				PolynomialFunctions.lowerIdentity(inputLength << 1).apply(
						concatenatedInput));
		Assert.assertEquals(
				inputUpper,
				PolynomialFunctions.upperIdentity(inputLength << 1).apply(
						concatenatedInput));

		BitVector combinationActual = stage.getCombination().apply(
				concatenatedInput);
		Assert.assertEquals(expected, combinationActual);

		BitVector overallActual = stage.getCombination().apply(
				stage.getStep().apply(input));
	...
	public void testCombination() {
		...
		EnhancedBitMatrix c1 = EnhancedBitMatrix
				.randomInvertibleMatrix(inputLength);
		EnhancedBitMatrix c2 = EnhancedBitMatrix
				.randomInvertibleMatrix(inputLength);

		SimplePolynomialFunction combination = PolynomialFunctions
				.linearCombination(c1, c2);
		...
		SimplePolynomialFunction f = c1.multiply(PolynomialFunctions
				.lowerIdentity(inputLength << 1));
		SimplePolynomialFunction g = c2.multiply(PolynomialFunctions
				.upperIdentity(inputLength << 1));
	...
	public void testUnitPipeline() {
		...
		SimplePolynomialFunction[] functions = PolynomialFunctions
				.arrayOfRandomMultivariateQuadratics(inputLength, outputLength,
						PIPELINE_LENGTH);
		...
		logger.info("Non-linear unit pipeline generation took {} ms",
				PIPELINE_LENGTH, watch.elapsed(TimeUnit.MILLISECONDS));

		CompoundPolynomialFunction originalPipeline = CompoundPolynomialFunctions
				.fromFunctions(functions);
		CompoundPolynomialFunction newPipeline = CompoundPolynomialFunctions
				.fromFunctions(pipelineDescription.getRight());

		BitVector input = BitUtils.randomVector(inputLength << 1);
		BitVector expected = originalPipeline.apply(inner.apply(input));
		BitVector actual = pipelineDescription.getLeft().apply(
				newPipeline.apply(input));
		Assert.assertEquals(expected, actual);
	...
	public void testParallelization() {
		...
		for (int i = 0; i < 100; ++i) {
			BitVector input = randomVector();
			BitVector innerInput = FunctionUtils.concatenate(randomVector(),
					randomVector());
			Assert.assertEquals(outerSlow.apply(input), outerFast.apply(input));
			Assert.assertEquals(innerSlow.apply(innerInput),
					innerFast.apply(innerInput));
			Assert.assertEquals(slowComposed.apply(innerInput),
					fastComposed.apply(innerInput));
		}
	}
	...
	public SimplePolynomialFunction denseRandomFunction() {
		return PolynomialFunctions.denseRandomMultivariateQuadratic(
	...
	public SimplePolynomialFunction linearFunction() {
		return PolynomialFunctions.randomManyToOneLinearCombination(
				INPUT_LENGTH).deoptimize();
	}
	...
	public void testRowProduct() {
		...
		OptimizedPolynomialFunctionGF2 function = new OptimizedPolynomialFunctionGF2(
				0, 0, null, null);
		...
		for (int i = 0; i < lhs.size(); ++i) {
			if (lhs.get(i)) {
				for (int j = 0; j < rhs.size(); j++) {
					if (rhs.get(j)) {
						Assert.assertNotNull(indices.get(mList.get(i).product(
								mList.get(j))));
					}

					if (i != j && lhs.get(i) && rhs.get(j) && lhs.get(j)
							&& rhs.get(i)) {
						Assert.assertFalse(p.get(indices.get(mList.get(i)
								.product(mList.get(j)))));
					}
				}
			}
	...
	public void mostFrequentFactorTest() {
		...
		Map<Monomial, Set<Monomial>> memoizedComputations = OptimizedPolynomialFunctionGF2
				.initializeMemoMap(256, monomials, contributions);
		...
		Monomial mostFrequent = OptimizedPolynomialFunctionGF2
				.mostFrequentFactor(monomials, possibleProducts.keySet(),
						ImmutableSet.<Monomial> of());
		logger.trace("Most frequent monomial found: {}", mostFrequent);
		Assert.assertEquals(new Monomial(256).chainSet(0).chainSet(1),
				mostFrequent);
	}
	...
	public void testConcatenateInputsAndOutputs() {
		...
		long[] src = new long[] { r.nextLong(), r.nextLong(), r.nextLong(),
				r.nextLong() };
	...
	public void testNonlinearPipeline() {
		...
		Pair<SimplePolynomialFunction, SimplePolynomialFunction[]> pipelineDescription = PolynomialFunctions
				.buildNonlinearPipeline(inner, functions);
		logger.info("Non-linear pipeline generation of length {} took {} ms",
				PIPELINE_LENGTH, watch.elapsed(TimeUnit.MILLISECONDS));
	...
	public void testSimplePolynomialFunctionMarshalWithNull() {
		Assert.assertNull(PolynomialFunctions
				.marshalSimplePolynomialFunction(null));
	}

	@Test
	public void testSimplePolynomialFunctionUnmarshalWithNull() {
		Assert.assertNull(PolynomialFunctions
				.unmarshalSimplePolynomialFunction(null));
	}
	...
	public void testSimplePolynomialFunctionMarshalUnmarshal() {
		SimplePolynomialFunction original = PolynomialFunctions.randomFunction(
				INPUT_LENGTH, OUTPUT_LENGTH);

		String data = PolynomialFunctions
				.marshalSimplePolynomialFunction(original);
		SimplePolynomialFunction result = PolynomialFunctions
				.unmarshalSimplePolynomialFunction(data);

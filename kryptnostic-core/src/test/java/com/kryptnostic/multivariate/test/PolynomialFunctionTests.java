...
public class PolynomialFunctionTests {
    ...
    public void mostFrequentFactorTest() {
        ...
        Map<Monomial, Set<Monomial>> memoizedComputations = PolynomialFunctionGF2.initializeMemoMap(256, monomials,
                contributions);
        Map<Monomial, List<Monomial>> possibleProducts = PolynomialFunctionGF2.allPossibleProduct(memoizedComputations
                .keySet()); // 1
        Monomial mostFrequent = PolynomialFunctionGF2.mostFrequentFactor(monomials, possibleProducts.keySet(),
    ...
    public void benchmarkAllPossibleProduct() {
        ...
        long totalNanos = 0, start = 0, stop = 0;
        double mu = 0D, sigma = 0D;
        long[] timings = new long[ROUNDS];

        for (int i = 0; i < timings.length; ++i) {
            SimplePolynomialFunction mvq = PolynomialFunctions.denseRandomMultivariateQuadratic(INPUT_LENGTH,
                    OUTPUT_LENGTH);
            Map<Monomial, Set<Monomial>> memoizedComputations = PolynomialFunctionGF2.initializeMemoMap(INPUT_LENGTH,
                    mvq.getMonomials(), mvq.getContributions());
            start = System.nanoTime();
            Map<Monomial, List<Monomial>> possibleProducts = PolynomialFunctionGF2
                    .allPossibleProduct(memoizedComputations.keySet());
        ...
        for (int i = 0; i < timings.length; ++i) {
            ...
            Map<Monomial, Set<Monomial>> memoizedComputations = PolynomialFunctionGF2.initializeMemoMap(INPUT_LENGTH,
                    mvq.getMonomials(), mvq.getContributions());
            start = System.nanoTime();
            // Map<Monomial, List<Monomial>> possibleProducts = PolynomialFunctionGF2.allPossibleProductParallelEx2(
            // memoizedComputations.keySet() , memoizedComputations.keySet() ,Monomials.deepCloneToImmutableSet(
            // mvq.getMonomials() ) , 2);
            stop = System.nanoTime();
            timings[i] = stop - start;
            totalNanos += timings[i];
    ...
    public void benchmarkMostFrequentFactor() {
        ...
        for (int i = 0; i < timings.length; ++i) {
            ...
            Map<Monomial, List<Monomial>> possibleProducts = PolynomialFunctionGF2
                    .allPossibleProduct(memoizedComputations.keySet());
            start = System.nanoTime();
            Monomial mostFreq = PolynomialFunctionGF2.mostFrequentFactor(mvq.getMonomials(), possibleProducts.keySet(),
                    memoizedComputations.keySet());
            stop = System.nanoTime();
            timings[i] = stop - start;
            totalNanos += timings[i];
        }
        ...
        for (int i = 0; i < timings.length; ++i) {
            ...
            Map<Monomial, Set<Monomial>> memoizedComputations = PolynomialFunctionGF2.initializeMemoMap(INPUT_LENGTH,
                    mvq.getMonomials(), mvq.getContributions());
            // Map<Monomial, List<Monomial>> possibleProducts = PolynomialFunctionGF2.allPossibleProductParallelEx2(
            // memoizedComputations.keySet() , memoizedComputations.keySet() , remaining , 2);
            start = System.nanoTime();
            // Monomial mostFreq = PolynomialFunctionGF2.mostFrequentFactorParallel( possibleProducts.keySet() ,
            // remaining );
            stop = System.nanoTime();
            timings[i] = stop - start;
            totalNanos += timings[i];
